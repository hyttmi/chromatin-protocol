...
void HttpRouter::add_route(std::string_view method, std::string_view prefix,
    ...
    routes_.push_back(Route{
        std::string(method),
        std::string(prefix),
        std::variant<Handler, AsyncHandler>{std::move(handler)},
        public_route
    });
}
...
bool prefix_matches(const std::string& path, const std::string& prefix) {
    return path == prefix ||
           (prefix.back() == '/' && path.size() > prefix.size() &&
            path.compare(0, prefix.size(), prefix) == 0);
}

/// Perform auth middleware check. Returns session pointer on success,
/// or an error HttpResponse if auth fails.
std::variant<HttpSessionState*, HttpResponse> check_auth(
    const HttpRequest& req, TokenStore& token_store) {
...
// ---------------------------------------------------------------------------
// Synchronous dispatch (unchanged behavior for sync-only callers)
// ---------------------------------------------------------------------------
...
HttpResponse HttpRouter::dispatch(const HttpRequest& req, const std::vector<uint8_t>& body,
    ...
    for (const auto& route : routes_) {
        if (!prefix_matches(req.path, route.prefix)) continue;
        prefix_matched = true;
        if (req.method != route.method) continue;

        // Only sync handlers can be dispatched synchronously.
        auto* sync_handler = std::get_if<Handler>(&route.handler);
        if (!sync_handler) continue;  // Skip async routes in sync dispatch

        if (!route.public_route) {
            auto auth_result = check_auth(req, token_store);
            if (auto* err = std::get_if<HttpResponse>(&auth_result)) {
                return *err;
            }
            return (*sync_handler)(req, body, std::get<HttpSessionState*>(auth_result));
        }

        return (*sync_handler)(req, body, nullptr);
    }

    return prefix_matched ? method_not_allowed() : HttpResponse::not_found();
}
...
asio::awaitable<HttpResponse> HttpRouter::dispatch_async(
    const HttpRequest& req, const std::vector<uint8_t>& body, TokenStore& token_store) {
    ...
    for (const auto& route : routes_) {
        ...
        if (!route.public_route) {
            ...
            session = std::get<HttpSessionState*>(auth_result);
        }

        // Dispatch to sync or async handler.
        if (auto* sync_handler = std::get_if<Handler>(&route.handler)) {
            co_return (*sync_handler)(req, body, session);
        }
        auto* async_handler = std::get_if<AsyncHandler>(&route.handler);
        co_return co_await (*async_handler)(req, body, session);
    }

    co_return prefix_matched ? method_not_allowed() : HttpResponse::not_found();
}
