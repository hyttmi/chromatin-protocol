...
void validate_config(const Config& cfg) {
    if (cfg.tcp_port == 0) {
        throw std::runtime_error("invalid tcp_port: must be 1-65535");
    }
    if (cfg.ws_port == 0) {
        throw std::runtime_error("invalid ws_port: must be 1-65535");
    }
    if (cfg.tcp_port == cfg.ws_port) {
        throw std::runtime_error("tcp_port and ws_port must be different");
    }
    if (cfg.bind.empty()) {
        throw std::runtime_error("bind address must not be empty");
    }
