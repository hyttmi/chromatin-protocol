...
asio::awaitable<IngestResult> BlobEngine::store_chunked(
    ...
    // Step 1: Split data into chunk payloads (D-01, D-02)
    ...
    for (size_t offset = 0; offset < data.size(); offset += CHUNK_SIZE) {
        ...
        chunk_payloads.emplace_back(data.begin() + offset,
                                    data.begin() + offset + len);
    }

    // Step 2: Build, sign, encode each chunk (D-06)
    std::vector<storage::PrecomputedBlob> precomputed;
    std::vector<std::array<uint8_t, 32>> chunk_hashes;
    precomputed.reserve(chunk_payloads.size() + 1);  // +1 for manifest
    chunk_hashes.reserve(chunk_payloads.size());

    for (auto& payload : chunk_payloads) {
        wire::BlobData chunk_blob;
        std::memcpy(chunk_blob.namespace_id.data(), namespace_id.data(), 32);
        chunk_blob.pubkey.assign(pubkey.begin(), pubkey.end());
        chunk_blob.data = std::move(payload);
        chunk_blob.ttl = ttl;
        chunk_blob.timestamp = timestamp;

        // Compute signing input and sign via callback (offloaded)
        auto signing_input = wire::build_signing_input(
            chunk_blob.namespace_id, chunk_blob.data, ttl, timestamp);
        chunk_blob.signature = co_await crypto::offload(pool_,
            [&signature_fn, si = std::move(signing_input)]() {
                return signature_fn(si);
            });

        // Encode and hash
        auto encoded = wire::encode_blob(chunk_blob);
        auto hash = wire::blob_hash(encoded);

        chunk_hashes.push_back(hash);
        precomputed.push_back({std::move(chunk_blob), hash, std::move(encoded)});
    }

    // Step 3: Build manifest (D-03, D-07)
    auto manifest_data_bytes = make_manifest_data(chunk_hashes);

    wire::BlobData manifest_blob;
    std::memcpy(manifest_blob.namespace_id.data(), namespace_id.data(), 32);
    manifest_blob.pubkey.assign(pubkey.begin(), pubkey.end());
    manifest_blob.data = std::move(manifest_data_bytes);
    manifest_blob.ttl = ttl;
    manifest_blob.timestamp = timestamp;

    auto manifest_signing_input = wire::build_signing_input(
        manifest_blob.namespace_id, manifest_blob.data, ttl, timestamp);
    manifest_blob.signature = co_await crypto::offload(pool_,
        [&signature_fn, si = std::move(manifest_signing_input)]() {
            return signature_fn(si);
        });

    auto manifest_encoded = wire::encode_blob(manifest_blob);
    auto manifest_hash = wire::blob_hash(manifest_encoded);

    precomputed.push_back({std::move(manifest_blob), manifest_hash,
                           std::move(manifest_encoded)});

    // Step 4: Atomic store (D-11)
    ...
    // Check the last result (manifest) for the overall outcome.
    // If ANY result failed, all are failed (atomic all-or-nothing).
    const auto& manifest_result = results.back();
    switch (manifest_result.status) {
        case storage::StoreResult::Status::Stored:
        case storage::StoreResult::Status::Duplicate: {
            WriteAck ack;
            ack.blob_hash = manifest_result.blob_hash;
            ack.seq_num = manifest_result.seq_num;
            ack.status = (manifest_result.status == storage::StoreResult::Status::Stored)
        ...
        case storage::StoreResult::Status::CapacityExceeded:
            co_return IngestResult::rejection(IngestError::storage_full,
                "storage capacity exceeded during chunked store");
        case storage::StoreResult::Status::QuotaExceeded:
            co_return IngestResult::rejection(IngestError::quota_exceeded,
                "namespace quota exceeded during chunked store");
        case storage::StoreResult::Status::Error:
            co_return IngestResult::rejection(IngestError::storage_error,
                "storage write failed during chunked store");
    }
...
ChunkedReadResult BlobEngine::read_chunked(
    ...
    ChunkedReadResult result;

    // Step 1: Fetch manifest blob
    auto manifest_opt = storage_.get_blob(namespace_id, manifest_hash);
    if (!manifest_opt) {
    ...
    auto hashes_opt = parse_manifest(manifest_opt->data);
    if (!hashes_opt) {
        result.error = "not a manifest blob";
        return result;
    }

    // Step 3: Set chunks_expected
    result.chunks_expected = static_cast<uint32_t>(hashes_opt->size());

    // Step 4: Fetch each chunk in order and reassemble
    std::vector<uint8_t> reassembled;
    for (const auto& chunk_hash : *hashes_opt) {
        auto chunk_opt = storage_.get_blob(namespace_id, chunk_hash);
        if (!chunk_opt) {
            result.error = "missing chunk " + util::to_hex(chunk_hash);
            return result;
        }
        result.chunks_found++;
        reassembled.insert(reassembled.end(),
                           chunk_opt->data.begin(), chunk_opt->data.end());
    }

    // Step 5: Return success
    result.success = true;
    result.data = std::move(reassembled);
    return result;
}
