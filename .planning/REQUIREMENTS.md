...
- [x] **BLOB-03**: Sync hash collection reads hashes from seq_map index without loading blob data
- [x] **BLOB-04**: Sync transfers blobs individually with batched requests capped by `MAX_HASHES_PER_REQUEST`
- [x] **BLOB-05**: Transport validates declared frame length against max before allocating receive buffer
- [x] **BLOB-06**: Sync timeout adapts to transfer size to prevent timeout on large blob exchanges
...
| BLOB-03 | Phase 11 | Complete |
| BLOB-04 | Phase 11 | Complete |
| BLOB-05 | Phase 11 | Complete |
| BLOB-06 | Phase 11 | Complete |
