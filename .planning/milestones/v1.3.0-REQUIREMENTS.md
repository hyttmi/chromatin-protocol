# Requirements Archive: v1.3.0 Protocol Concurrency & Query Foundation
...
- [x] **CONC-01**: Transport envelope includes a `request_id: uint32` field that clients set on requests and the node echoes on corresponding responses
- [x] **CONC-02**: `DecodedMessage`, `TransportCodec`, `Connection::send_message`, and `MessageCallback` all carry `request_id` through the full encode/decode/dispatch pipeline
...
- [x] **CONC-05**: Relay forwards `request_id` bidirectionally in both `handle_client_message` and `handle_node_message` paths
...
|-------------|-------|--------|
| CONC-01 | Phase 61 | Complete |
| CONC-02 | Phase 61 | Complete |
| CONC-03 | Phase 62 | Complete |
| CONC-04 | Phase 62 | Complete |
| CONC-05 | Phase 61 | Complete |
