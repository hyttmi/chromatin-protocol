...
- [ ] **v1.5.0 Documentation & Distribution** — Phases 68-69 (in progress)
...
### v1.5.0 Documentation & Distribution (In Progress)

**Milestone Goal:** Complete documentation refresh and production distribution package. Zero C++ code changes.

- [ ] **Phase 68: Production Distribution Kit** - dist/ directory with systemd units, default configs, sysusers.d, tmpfiles.d, and install script for bare-metal deployment
...
**Goal**: Operators can deploy chromatindb from build output to running production service using a single install script
**Depends on**: Nothing (first phase of v1.5.0)
**Requirements**: DIST-01, DIST-02, DIST-03, DIST-04, DIST-05, DIST-06, DIST-07
**Success Criteria** (what must be TRUE):
  1. Running `sudo dist/install.sh` on a clean Debian Bookworm system creates the chromatindb user/group, directory structure, and installs both binaries with service files
  2. `systemctl start chromatindb` launches the node daemon and `systemctl status` shows "active (running)"
...
**Success Criteria** (what must be TRUE):
  1. README.md shows correct version, LOC count, test count, and lists all 58 message types
  2. README.md includes a relay deployment section explaining the relay+node architecture
  3. README.md includes dist/ deployment instructions referencing the install script and systemd units
  4. PROTOCOL.md documents all 58 message types with verified byte-level wire formats matching the encoder source code
  5. db/README.md reflects current state (version, test count, architecture)
**Plans**: TBD
